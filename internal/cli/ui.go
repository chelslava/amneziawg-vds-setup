...
func tr(lang uiLanguage, key string) string {
	if lang == langRU {
		if value, ok := map[string]string{
			...
			"select_action":           "Выберите действие", "goodbye": "До свидания.", "invalid_menu": "Выберите число от 0 до 7.", "cancelled": "Отменено.", "operation_failed": "Операция завершилась ошибкой:", "operation_completed": "Операция завершена.", "return_menu": "Возврат в главное меню…", "panel_password_unavailable": "Пароль панели не удалось получить автоматически; используйте SSH-команду из README.", "proceed": "Продолжить", "engine": "Движок", "vpn_port": "UDP-порт VPN", "panel_port": "TCP-порт панели", "domain": "Домен (пусто = HTTP по адресу VDS)", "enable_tls": "Включить TLS через Caddy", "panel_restriction": "Ограничение панели по IP (необязательно)", "vds_address": "Адрес VDS", "vds_required": "адрес VDS обязателен", "ssh_user": "Пользователь SSH", "ssh_port": "Порт SSH", "ssh_auth": "Метод SSH-аутентификации", "identity_file": "Файл SSH-ключа", "known_hosts": "Файл known_hosts (пусто = системный)", "password_notice": "Пароль будет запрошен один раз самим awg-vds без отображения символов; он хранится только в памяти и не сохраняется.",
		}[key]; ok {
			return value
		}
	}
