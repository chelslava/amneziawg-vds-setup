...
const Image = config.LegacyImage

type Engine struct{}

func (Engine) Name() config.Engine                   { return config.Legacy }
func (Engine) Image() string                         { return Image }
func (Engine) Container() string                     { return "awg-vds-legacy" }
...
func runCommand(s state.State, first bool) string {
	...
	return install + fmt.Sprintf("docker pull %s; docker rm -f %s >/dev/null 2>&1 || true; docker run -d --name %s --network host --env PORT=%d --env WG_PORT=%d --env WG_PERSISTENT_KEEPALIVE=25 --env WG_DEFAULT_DNS=1.1.1.1,1.0.0.1 --env EXPERIMENTAL_AWG=false --env-file /opt/awg-vds/legacy.env -v /opt/awg-vds/wireguard:/etc/amnezia/amneziawg -v /opt/awg-vds/wireguard:/etc/wireguard --cap-add=NET_ADMIN --cap-add=SYS_MODULE --device /dev/net/tun:/dev/net/tun --restart unless-stopped %s", quote(image), quote(s.Container), quote(s.Container), s.WebPort, s.VPNPort, quote(image))
}
