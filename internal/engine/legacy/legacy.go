...
func runCommand(s state.State, first bool) string {
	...
	return install + fmt.Sprintf("docker pull %s; docker rm -f %s >/dev/null 2>&1 || true; docker run -d --name %s --network host --env WG_HOST=%s --env PORT=%d --env WG_PORT=%d --env WG_PERSISTENT_KEEPALIVE=25 --env WG_DEFAULT_DNS=1.1.1.1,1.0.0.1 --env EXPERIMENTAL_AWG=false --env-file /opt/awg-vds/legacy.env -v /opt/awg-vds/wireguard:/etc/amnezia/amneziawg -v /opt/awg-vds/wireguard:/etc/wireguard --cap-add=NET_ADMIN --cap-add=SYS_MODULE --device /dev/net/tun:/dev/net/tun --restart unless-stopped %s", quote(Image), quote(s.Container), quote(s.Container), quote(s.Domain), s.WebPort, s.VPNPort, quote(Image))
}
