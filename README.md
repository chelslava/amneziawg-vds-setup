...
При установке панель получает случайный bcrypt-пароль: исходный пароль хранится только на VDS в /opt/awg-vds/panel-password с 0600 и не попадает в state, operation logs или флаговый CLI-вывод. После успешного интерактивного `install` пароль один раз показывается на финальном TUI-экране; сохраните его в менеджере паролей. Повторно получить его можно отдельной SSH-командой `ssh root@HOST sudo cat /opt/awg-vds/panel-password`.
