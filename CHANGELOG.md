...
- **TCPMSS Clamping:** Автоматическое ограничение размера TCP-сегментов (`TCPMSS --clamp-mss-to-pmtu` и `--set-mss 1240`) в цепочке FORWARD, устраняющее фрагментацию пакетов, микропотери (packet loss) и деградацию скорости на клиентах с MTU 1280 в сотовых сетях (4G LTE).
- **Сетевой тюнинг и BBR:** Увеличение длины очереди интерфейса wg0 (`txqueuelen 2000`), системных буферов сокетов до 16 МБ (`net.core.rmem_max=16M`, `wmem_max=16M`) и активация алгоритма контроля перегрузок `BBR + fq`.
- **Локальный кэширующий DNS:** Развёртывание изолированного сервиса `vpn-dnsmasq.service` на шлюзе `10.8.0.1:53` с кэшем на 10 000 записей (отклик 0 мс), снижающего задержки загрузки веб-страниц.
- **Отказоустойчивость памяти (Swap):** Автоматическое создание и активация 1 ГБ swap-файла (`/swapfile`) с `vm.swappiness=10` при его отсутствии в системе для защиты от OOM-killer на VDS с объёмом RAM 1 ГБ.
- **Оптимизация обфускации для сотовых сетей:** Фиксация безопасного `WG_MTU=1280` и компактного размера мусорных пакетов (`Jmax` до 150 байт), предотвращающего сброс фрагментированного UDP мобильными операторами.
- **Dual-Port Redirection:** Прозрачный редирект в iptables между портами 443 и 1234 для бесшовной миграции клиентов на мимикрирующий порт UDP 443 (HTTP/3 QUIC) без разрыва соединений на существующих профилях.

## v2.6.0 — 2026-09-07

- Персистентность маршрутизации и NAT: правила FORWARD и MASQUERADE для интерфейса wg0 теперь сохраняются в `/opt/awg-vds/bin/apply-nat.sh` и активируются через systemd-сервис `awg-vds-forwarding.service`, сохраняя доступ клиентов в интернет после перезагрузок VDS (#18).
- Совместимость с firewalld на RPM-дистрибутивах: перманентно настраивается `--add-masquerade` и доверенная зона для интерфейса wg0 (`--zone=trusted --add-interface=wg0`), благодаря чему `firewall-cmd --reload` больше не сбрасывает сетевой трафик VPN-клиентов (#19).
- Очистка контейнеров при сбое установки: при неудачном initial install созданные контейнеры автоматически удаляются, устраняя блокировку портов (`PORT_TCP_...=busy`) при последующих повторных попытках установки (#20).
- Исправлена предпроверка занятости портов: preflight теперь парсит локальный адрес из 4-й колонки `ss -lunH`, а валидатор `install` проверяет занятость как TCP, так и UDP портов (#21).
- Изоляция команды диагностики в TUI: запуск `Run doctor` из меню восстановления больше не мутирует команду установки и не приводит к ложному завершению операции, сохраняя возможность повторить установку через `Retry` (#22).

- Health-check теперь требует три последовательных успешных проверки контейнера,
  HTTP, TCP/UDP и AWG, а внешний отказ панели выводит отдельные рекомендации по
  firewall, security group и bind address.
- После успешного интерактивного `install` финальный TUI-экран показывает
  сохраняемую карточку доступа: URL панели, отсутствие логина, пароль, VPN-порт,
  engine, SSH и команду повторного чтения; operation logs, state и флаговый CLI
  по-прежнему не содержат пароль.
...
- Исправлена установка зависимостей на Debian: Compose больше не считается
  ...
  `docker-ce-cli` или `docker-cli`, при повреждённой установке выполняется
  `--reinstall`, затем CLI проверяется до запуска engine.
- Исправлен Debian/Ubuntu Docker install: `docker.io` может содержать daemon и
  CLI без отдельного `docker-cli`; установщик больше не обрывает dependency step,
  если отдельный CLI-пакет отсутствует, и проверяет реальную команду `docker`.
...
- Для `install` стандартный SSH timeout увеличен до 30 минут, чтобы полный upgrade,
  `dnf install` и DKMS-сборка не обрывались на малых VDS; `--timeout` остаётся доступен.
